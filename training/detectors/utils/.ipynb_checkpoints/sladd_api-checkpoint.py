...
def copy_fake_to_real(realimg, fakeimg, mask, mag):
    ...
    blend_list = [0.25, 0.5, 0.75,0.75, 1, 1, 1]
    blend_ratio = blend_list[np.random.randint(len(blend_list))]
    mask_blured *= blend_ratio
    img_blended = fakeimg*mask_blured * mag + realimg*(1-mask_blured)+realimg * mask_blured * (1-mag)
    # img_blended = (mask_blured * realimg + (1 - mask_blured) * fakeimg)
    return img_blended
