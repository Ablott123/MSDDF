...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def clip_keypoints(self,augmented_landmark, img_height, img_width):
        """
        Clip keypoints to ensure they fall within the image boundaries.

        Args:
            augmented_landmark: Numpy array of augmented landmarks.
            img_height: Height of the image.
            img_width: Width of the image.

        Returns:
            Clipped landmarks ensuring all coordinates are within [0, img_height] for y and [0, img_width] for x.
        """
        # Ensure landmarks are within the image bounds
        augmented_landmark[:, 0] = np.clip(augmented_landmark[:, 0], 0, img_width - 1)  # Clip x-coordinates
        augmented_landmark[:, 1] = np.clip(augmented_landmark[:, 1], 0, img_height - 1)  # Clip y-coordinates
        return augmented_landmark
    ...
    def data_aug(self, img, landmark=None, mask=None, augmentation_seed=None):
        ...
        if landmark is not None:
            landmark = self.clip_keypoints(landmark, img_height, img_width)
